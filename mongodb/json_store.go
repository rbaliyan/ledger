...
func (s *JSONStore) Stat(ctx context.Context, stream string) (ledger.StreamStat[string], error) {
	if s.closed.Load() {
...
func (s *JSONStore) Search(ctx context.Context, stream string, query string, opts ...ledger.ReadOption) ([]ledger.StoredEntry[string, json.RawMessage], error) {
	if s.closed.Load() {
		return nil, ledger.ErrStoreClosed
	}
